defmodule NgrokTest do
  ...
  test "it stores the settings", %{ngrok_name: ngrok_name} do
    ...
    assert Ngrok.public_url(ngrok_name) =~ ~r/http(s)?:\/\/(.*)\.ngrok-free\.app/
  end
  ...
  test "it raises when it cannot connect to the Ngrok API", %{ngrok_name: ngrok_name} do
    ...
    assert {{:shutdown,
             {:failed_to_start_child, Ngrok.Settings,
              {%RuntimeError{
                 message:
                   "Unable to retrieve setting from Ngrok: Could not connect to Ngrok API on http://localhost:0, reason: :eaddrnotavail"
               }, _}}}, _} = error
  end
